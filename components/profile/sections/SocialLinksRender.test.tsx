...
describe('SocialLinksRender', () => {
  it('renders nothing when there are no links', () => {
    const { container } = render(<SocialLinksRender bundle={makeBundle([])} />);
  ...
  it('emits mailto: for email and opens in same tab', () => {
    render(
      <SocialLinksRender
        bundle={makeBundle([
    ...
    const link = screen.getByRole('link', { name: /e-mail/i });
    expect(link).toHaveAttribute('href', 'mailto:dj@example.com');
    expect(link).not.toHaveAttribute('target');
  });

  it('emits wa.me for whatsapp and opens in same tab', () => {
    render(
      <SocialLinksRender
        bundle={makeBundle([
  ...
  it('coerces a bare email string into mailto:', () => {
    render(
      <SocialLinksRender
        bundle={makeBundle([
