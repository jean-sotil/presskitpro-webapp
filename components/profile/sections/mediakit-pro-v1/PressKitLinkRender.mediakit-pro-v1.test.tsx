...
import { PressKitLinkMediakitProV1 } from './PressKitLinkRender.mediakit-pro-v1';
...
describe('PressKitLinkMediakitProV1', () => {
  it('renders the Download press kit heading + CTA', async () => {
    await renderAsync(
      PressKitLinkMediakitProV1({
        bundle: makeBundle({
  ...
  it('returns null when there is no URL', async () => {
    const { container } = await renderAsync(
      PressKitLinkMediakitProV1({ bundle: makeBundle({}) }),
    );
    expect(container).toBeEmptyDOMElement();
  });
});
