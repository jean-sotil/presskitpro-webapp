...
/**
 * MediakitPRO biography panel — split 40/60 layout.
 *
 ...
 * display heading, then the rich-text bio. The first paragraph gets
 * a heavier weight + accent left-border to mimic the spec's
 * "first-paragraph emphasis" without parsing Lexical state.
 *
 * Mobile (single column): text first, image below — set via grid
 * `col-start` / `row-start` overrides at the `md:` breakpoint.
 *
 * The festival-club-orange preset ships an identical visual; both are
 * folder-owned so each preset carries its own copy. Future divergence
 * (e.g. brighter palette tweaks per preset) can land without churning
 * the other one.
 */
export function AboutMediakitProV1({ bundle }: { bundle: EditorBundle }) {
