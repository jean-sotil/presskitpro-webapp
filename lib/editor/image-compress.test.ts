...
describe('compressImage', () => {
  ...
  it('pipes a candidate JPEG through canvas and returns a new File', async () => {
    const input = makeFile('hero.jpg', 'image/jpeg', 5 * 1024 * 1024);
    const compressedBlob = new Blob([new Uint8Array(1024 * 1024)], { type: 'image/jpeg' });
    const deps: CompressDeps = {
      loadImage: vi.fn().mockResolvedValue({ width: 4800, height: 3600 }),
      drawAndExport: vi.fn().mockResolvedValue(compressedBlob),
    };
    const out = await compressImage(input, { maxEdge: 2400, quality: 0.82 }, deps);
    expect(out).toBeInstanceOf(File);
    expect(out.type).toBe('image/jpeg');
    expect(out.name).toBe('hero.jpg');
    expect(out.size).toBe(compressedBlob.size);
    expect(deps.drawAndExport).toHaveBeenCalledWith({
      image: { width: 4800, height: 3600 },
      width: 2400,
      height: 1800,
      mimeType: 'image/jpeg',
      quality: 0.82,
  ...
  it('returns the original file when the compressed output is somehow LARGER (defensive)', async () => {
    ...
    const oversized = new Blob([new Uint8Array(200_000)], { type: 'image/jpeg' });
    const deps: CompressDeps = {
