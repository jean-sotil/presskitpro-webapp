...
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   CREATE TYPE "payload"."enum_users_role" AS ENUM('user', 'admin');
  CREATE TYPE "payload"."enum_users_plan" AS ENUM('free', 'pro');
  CREATE TYPE "payload"."enum_profiles_status" AS ENUM('draft', 'published', 'unpublished');
  CREATE TYPE "payload"."enum_media_bucket" AS ENUM('avatars', 'gallery');
  ...
  CREATE TABLE "payload"."users" (
  	...
  	"plan" "payload"."enum_users_plan" DEFAULT 'free' NOT NULL,
  ...
  CREATE TABLE "payload"."profiles" (
  	...
  	"status" "payload"."enum_profiles_status" DEFAULT 'draft' NOT NULL,
  	"updated_at" timestamp(3) with time zone DEFAULT now() NOT NULL,
  	"created_at" timestamp(3) with time zone DEFAULT now() NOT NULL
  );
  ...
  CREATE TABLE "payload"."media" (
  	"id" serial PRIMARY KEY NOT NULL,
  	"bucket" "payload"."enum_media_bucket" NOT NULL,
  ...
  ALTER TABLE "payload"."profiles" ADD CONSTRAINT "profiles_owner_id_users_id_fk" FOREIGN KEY ("owner_id") REFERENCES "payload"."users"("id") ON DELETE set null ON UPDATE no action;
  ALTER TABLE "payload"."media" ADD CONSTRAINT "media_owner_id_users_id_fk" FOREIGN KEY ("owner_id") REFERENCES "payload"."users"("id") ON DELETE set null ON UPDATE no action;
  ...
  CREATE UNIQUE INDEX "users_supabase_user_id_idx" ON "payload"."users" USING btree ("supabase_user_id");
  CREATE INDEX "users_email_idx" ON "payload"."users" USING btree ("email");
  CREATE INDEX "users_updated_at_idx" ON "payload"."users" USING btree ("updated_at");
  CREATE INDEX "users_created_at_idx" ON "payload"."users" USING btree ("created_at");
  CREATE INDEX "profiles_owner_idx" ON "payload"."profiles" USING btree ("owner_id");
  ...
  CREATE INDEX "media_path_idx" ON "payload"."media" USING btree ("path");
  CREATE INDEX "media_owner_idx" ON "payload"."media" USING btree ("owner_id");
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  DROP TYPE "payload"."enum_users_plan";
  DROP TYPE "payload"."enum_profiles_status";
  DROP TYPE "payload"."enum_media_bucket";`)
}
