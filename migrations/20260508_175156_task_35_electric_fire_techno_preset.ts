 ...
 * `electric-fire-techno` preset (near-black + fire-orange aesthetic).
 * Additive change: every existing row's `preset_id` is unaffected.
 */
export async function up({ db, payload, req }: MigrateUpArgs): Promise<void> {
  await db.execute(sql`
   ALTER TYPE "payload"."enum_themes_preset_id" ADD VALUE 'electric-fire-techno' BEFORE 'editorial-nightlife-v1';`)
}
...
export async function down({ db, payload, req }: MigrateDownArgs): Promise<void> {
  ...
  CREATE TYPE "payload"."enum_themes_preset_id" AS ENUM('mediakit-pro-v1', 'festival-club-orange', 'editorial-nightlife-v1');
