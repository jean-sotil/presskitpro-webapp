...
- A profile is valid if it has content in _at least one_ locale; missing translations fall back to the profile's `defaultLocale` with a `lang` attribute correctly set on the rendered region.
...
- `accent` — pick from 12 curated accents (electric green, hot pink, signal red, amber, ice blue, etc.) or custom hex. _Validation: must pass WCAG AA contrast against the chosen `bg`. The picker shows a live contrast badge._
...
- A list of 8 hand-picked display+body pairs, each with a clear personality:
  - "Editorial Nightlife" (Anton + Manrope) — _default_
