...
- **R1 — `/checkout/*` 404s today.** The pricing CTAs point at routes that don't exist until task-23. _Mitigation:_ documented in the runbook. Logged-out flow correctly reaches `/login?next=/checkout/...` which then 404s post-login — acceptable until task-23 lands.
- **R2 — Annual price drift.** The toggle swaps displayed values but the CTA stays monthly. _Mitigation:_ a clear "Anual em breve" hint when the toggle is on; the CTA href stays on the monthly Stripe ID.
- **R3 — Stripe Price IDs leak via NEXT*PUBLIC*\*.** Stripe Price IDs are not secrets but they shape what users can purchase. _Mitigation:_ the env vars are server-only (no `NEXT_PUBLIC_` prefix); the page passes only the resolved CTA href down to client components.
