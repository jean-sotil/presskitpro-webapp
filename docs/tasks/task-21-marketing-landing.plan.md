...
- **R1 — `/pricing` 404s.** Hero teaser links to a route that doesn't exist until task-22. _Mitigation:_ document in runbook; the click still gets visitors who'll see the standard 404. Acceptable for the dev period.
- **R2 — Empty live-examples grid in fresh dev DBs.** No published profiles → carousel renders an empty state. _Mitigation:_ the carousel's empty state copy points the dev at `pnpm seed`.
- **R3 — Locale toggle visually misleading.** It's inert until task-29; users may think the toggle is broken. _Mitigation:_ it's labelled with the current locale (PT-BR active) and a `title="Em breve"` hint.
