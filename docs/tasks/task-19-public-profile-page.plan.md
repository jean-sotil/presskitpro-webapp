...
- **R1 — Catch-all collides with future routes.** Adding `app/[slug]/page.tsx` shadows any non-reserved single-segment top-level path. _Mitigation:_ slug validation (task-07) already prohibits the routes we own; new top-level routes need a real-path declaration (e.g. `app/explore/page.tsx`).
...
- **R3 — Trial-paused profiles get the wrong UX.** They 404 instead of seeing the paused page. _Mitigation:_ TODO marker in the route + plan reference to task-31.
